	...
	class LoginDoc{
        ...
        */
        public function SynchEntries(&$LIerror){
            ...
            $query="SELECT doc_password FROM DocInfo WHERE doc_email=?";
        ...
        */
        protected function CreateSession(){
            ...
            $this->connToDB();
            //prepared statements
            $query="SELECT doc_name, doc_RegNo, doc_department 
                    FROM DocInfo WHERE doc_email=?";
            $stmt=($this->conn)->prepare($query);
            $stmt->bind_param('s',$this->userID);
            $stmt->execute();
            $stmt->store_result();
            if($stmt->num_rows){
                $stmt->bind_result($name,$RegNo,$Depmnt);
                $stmt->fetch();
                
                $_SESSION['name']=htmlentities($name);
                $_SESSION['RegNo']=htmlentities($RegNo);
                $_SESSION['depmnt']=htmlentities($Depmnt);
        ...
        */
        public function CheckCookie(){
            if(isset($_COOKIE['user'])){            //Checks Whether Cookie Exists
                ...
                $query="SELECT doc_email FROM DocInfo WHERE doc_session=?";
                $stmt=($this->conn)->prepare($query);
            ...
            if(isset($_SESSION['DocLogIN'])){
                if($_SESSION['DocLogIN']===200){
                    ...
                    if($cookRep){
                        ...
                        //Insert Session Key
                        $this->connToDB();
                        $query="UPDATE DocInfo SET
                            doc_session=? WHERE doc_email=?";
                        $stmt=($this->conn)->prepare($query);
